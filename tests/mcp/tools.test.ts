...
describe("MCP tools", () => {
  it("registers the fifteen read-only tools and exactly one write tool", () => {
    const names = listTools().map((tool) => tool.name);
    expect(names).toEqual([
      "shipready.audit_site", "shipready.crawl_site", "shipready.search_console_status", "shipready.dns_status", "shipready.recheck", "shipready.social_preview", "shipready.generated_site_smells", "shipready.export_patch", "shipready.github_pr_draft", "shipready.inspect_repo",
      "shipready.plan_fixes", "shipready.preview_fixes", "shipready.write_safe_crawl_files", "shipready.get_ui_report",
  ...
  it("returns an inline GitHub PR draft contract without writing artifacts", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "shipready-mcp-github-pr-draft-"));
    const repoPath = join(tempRoot, "repo");
    await cp(join(fixtureRoot, "next-app-router-dry-run"), repoPath, { recursive: true });
    try {
      const isolated = { ...context, authorizer: await PathAuthorizer.create([tempRoot]) };
      const before = await treeDigest(repoPath);
      const result = await callReadOnlyTool(isolated, "shipready.github_pr_draft", {
        url,
        repoPath,
      ...
      expect(contract).toMatchObject({
        ...
        output: { kind: "inline", wroteArtifact: false, bytesWritten: 0 },
        safety: { createdPullRequest: false, ranGitCommands: false, calledGitHubApi: false },
      });
      expect(after).toBe(before);
      expect(await readdir(tempRoot)).toEqual(["repo"]);
    } finally {
  ...
  it("normalizes invalid input, unknown fields, authorization, and timeouts", async () => {
    ...
    const outsidePrDraft = await callReadOnlyTool(context, "shipready.github_pr_draft", {
      url,
      repoPath: tmpdir(),
      rendered: false,
    });
    ...
    expect(outsidePrDraft).toMatchObject({ isError: true, structuredContent: { code: "path_not_authorized" } });
