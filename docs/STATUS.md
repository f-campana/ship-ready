...
- Local MCP stdio server with twelve read-only tools, one guarded safe-write tool for V1 crawl-file creations, canonical docs/fixtures, five prompts, explicit allowed roots, stable errors, preview receipts, and bounded deadlines.
...
- Multi-page crawl, terminal output polish/TUI viewer, or patch export.
...
**Pass 15: Bounded multi-page crawl.** Expand read-only audit scope after the generated-site implementation smell detector shipped without changing the current GUI or write policy.
