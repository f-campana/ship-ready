...
- Package publish decision with live npm registry checks, recommended future scoped package `@f-campana/shipready`, license blocker, no-`postinstall` browser decision, CLI-only metadata decision, publish authority/process, rollback/deprecate criteria, and required package smoke automation before any publish execution.
...
**Package publish blockers closure.** The package is packable and locally smoke-testable from a tarball, and the publish decision recommends `@f-campana/shipready` only if a future public package is explicitly approved. ShipReady v0 remains source-checkout-only. The next pass should close or explicitly defer license approval, npm scope ownership, exact package metadata changes, trusted-publishing or token process, installed usage docs, browser install verification, local/CI packed-package smoke automation, rollback/deprecate checklist, and GitHub tag/release policy. Do not publish, claim `pnpm dlx`, adopt Ink or OpenTUI, broaden `WRITE_POLICY_V1`, or change JSON/write/product behavior without a separate approved pass.
