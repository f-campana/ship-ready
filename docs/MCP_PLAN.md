...
Pass 13 adds the read-only `shipready.social_preview` tool around the CLI social preview simulator. It accepts only one URL plus optional source/check/mock inputs, needs no repository authorization, and performs no social platform API call, screenshot rendering, image generation, deployment, DNS/Search Console mutation, token handling, or filesystem write.
...
| `shipready.crawl.v1` | `crawl --url <url> --json` | `crawlSite` | `shipready.crawl_site` |
...
### 5.2 `shipready.crawl_site`

- Purpose/classification: run a bounded same-origin launch-readiness crawl; network-read-only or deterministic local mock.
- Input schema:
...
```json
{
  ...
  "properties": {
    ...
    "source": { "type": "string", "enum": ["sitemap", "links", "both"], "default": "both" },
    "rendered": { "type": "boolean", "default": true },
    "mock": {
      "type": "string",
      "enum": [
...
- Normalization: trim and normalize `url`; remove fragments and sensitive query output; reject embedded credentials; cap `maxPages` to 25 and `maxDepth` to 2; default `source` to `both` and `rendered` to `true`.
- Output: exact `shipready.crawl.v1`, validated by `CrawlJsonContractSchema`.
- Mapping: `crawl --url <url> --json [--max-pages] [--max-depth] [--source] [--no-render] [--mock]` / `crawlSite` / `formatCrawlJson`.
...
### 5.10 `shipready.dns_status`

- Purpose/classification: return read-only DNS readiness evidence for one HTTP(S) URL; network-read-only in live mode and deterministic local-read-only in mock mode.
- Input: `{ "url": string, "expectedCanonicalHost"?: string, "expectedWwwMode"?: "apex" | "www" | "either", "expectedSearchConsoleVerificationTxt"?: string, "checkHttp"?: boolean, "mock"?: "ready" | "apex-ok-www-missing" | "www-cname-ok" | "nxdomain" | "nodata" | "timeout" | "cname-chain-issue" | "caa-present" | "txt-found" | "txt-missing" | "canonical-mismatch" }`. Additional fields, including credential/provider/token fields, are rejected.
- Normalization: validates and normalizes one HTTP(S) URL through the CLI application boundary, strips query strings/fragments from output, derives conservative apex/www hosts, and maps `expectedSearchConsoleVerificationTxt` to the CLI's expected TXT input. No repo path is accepted or authorized.
- Output: exact `shipready.dnsStatus.v1`, validated by `DnsStatusJsonContractSchema`.
- Mapping: `dns status --url <url> [--mock <scenario>] [--expected-canonical-host <host>] [--expected-www-mode <mode>] [--expected-search-console-txt <token>] [--check-http] --json` / `getDnsStatus` / `formatDnsStatusJson`.
- Success: omitted `mock` performs read-only live DNS lookups through Node built-ins; every mock scenario is deterministic; TXT token values are matched internally and redacted from output.
- Errors: invalid URL is `invalid_url`; unsupported scenario or expected-www mode is `invalid_mode`; extra input is `unsupported_command`; contract drift is `contract_error`.
...
### 5.12 `shipready.social_preview`

- Purpose/classification: simulate likely search/social preview inputs from observed raw/rendered page metadata; network-read-only in live mode and deterministic local-read-only in mock mode.
- Input: `{ "url": string, "source"?: "raw" | "rendered" | "both", "checkAssets"?: boolean, "mock"?: "complete" | "missing-image" | "rendered-only-metadata" | "twitter-fallback" | "missing-description" | "missing-og-url" | "raw-rendered-different" | "image-unreachable" | "minimal-title-only" }`. Additional fields, including credentials, platform choices, provider tokens, repository paths, write flags, and deployment controls, are rejected.
- Normalization: validates and normalizes one HTTP(S) URL through the CLI application boundary. `source` defaults to `both`; URL-only calls do not authorize a repository.
- Output: exact `shipready.socialPreview.v1`, validated by `SocialPreviewJsonContractSchema`.
- Mapping: `social-preview --url <url> [--source raw|rendered|both] [--check-assets] [--mock <scenario>] --json` / `getSocialPreview` / `formatSocialPreviewJson`.
- Success: emitted results are simulated previews and may have `ready`, `needs_attention`, or `unknown` verdicts. Missing fields, rendered-only metadata, fallback use, or unreachable mock image evidence remain contract data, not tool errors.
- Errors: invalid URL is `invalid_url`; unsupported source or mock scenario is `invalid_mode`; extra input is `unsupported_command`; contract drift is `contract_error`.
- Timeout: 30 seconds at the MCP boundary. Mock mode performs no I/O. Live mode reuses the existing single-page audit path.
- Safety: no repository path, allowed-root authorization, filesystem mutation, social platform API, platform preview scraping endpoint, screenshot rendering, image generation, OAuth/token handling, DNS/Search Console mutation, Git/GitHub, deployment, provider integration, or extra MCP write tool exists. The result is an approximation based on observed metadata, not a third-party rendering guarantee.

### 5.13 `shipready.generated_site_smells`

- Purpose/classification: detect generated-site implementation smells in one authorized repository, with optional URL cross-check evidence; local-read-only and optional network-read-only.
- Input: `{ "repoPath": string, "url"?: string, "mock"?: "clean" | "vite-client-only-metadata" | "placeholder-content" | "missing-social-assets" | "hardcoded-localhost" | "unsupported-framework" | "repo-plus-url-rendered-only" | "multiple-smells" }`. Additional fields, including credentials, write flags, provider choices, platform choices, or generated-content instructions, are rejected.
- Normalization: `repoPath` must pass the section 7 absolute canonical allowed-root authorization before repository scanning. When `url` exists, it is normalized as HTTP(S) and query strings/fragments are omitted from output. Mock mode remains deterministic and local after path authorization.
- Output: exact `shipready.generatedSiteSmells.v1`, validated by `GeneratedSiteSmellsJsonContractSchema`.
- Mapping: `smells <path> [--url <url>] [--mock <scenario>] --json` / `getGeneratedSiteSmells` / `formatGeneratedSiteSmellsJson`.
- Success: clean, needs-attention, manual-review, unknown-framework, bounded-scan-limit, and mock results are successful contract data, not tool errors.
- Errors: invalid repo path is `invalid_repo_path`; unauthorized repo path is `path_not_authorized`; invalid URL is `invalid_url`; unsupported mock is `invalid_mode`; extra input is `unsupported_command`; contract drift is `contract_error`.
...
The package publish preparation pass verified that this resource model survives a clean packed-tarball install when the package includes `README.md`, `docs/`, `skills/shipready-launch-readiness/`, and `validation/contracts/`. The package publish decision did not add remote MCP transport, new MCP write tools, or arbitrary document paths. `docs/PACKAGE_PUBLISH_PREPARATION.md` and `docs/PACKAGE_PUBLISH_DECISION.md` are packaged as documentation but are not new MCP resource URIs unless a later pass explicitly expands the allowlist.
...
| `dns_status` | 5 s | Deferred; live DNS resolver calls are bounded internally |
| `social_preview` | 30 s | Deferred; live mode reuses the single-page audit timeout |
| `generated_site_smells` | 45 s | Deferred; URL mode reuses the single-page audit timeout |
...
tests/mcp.socialPreview.test.ts
tests/mcp.generatedSiteSmells.test.ts
```
...
Pass 12 added only the read-only post-write recheck described above. Pass 13 added only the read-only social preview simulator described above. Pass 14 added only the read-only generated-site implementation smell detector described above. Pass 15 added only the read-only bounded multi-page crawl described above. Pass 17 added only the review-only patch export described above. Pass 18 added only the review-only GitHub PR draft handoff described above. The roadmap closure / release-readiness review is complete in [RELEASE_READINESS.md](RELEASE_READINESS.md). The distribution decision is complete in [DISTRIBUTION.md](DISTRIBUTION.md): MCP remains repository-local stdio-only until installed package behavior is explicitly verified. Terminal output polish is complete for existing human CLI commands. The read-only TUI viewer is implemented as a CLI human surface only; no MCP tool, transport, authorization, or write behavior changed. Package publish preparation, package publish decision, publish readiness closure, and publish preflight wiring did not change MCP tools, transport, authorization, or write behavior. Package-root lookup now recognizes only the approved current/future ShipReady identities with canonical markers and keeps resource allowlists unchanged. The recommended next pass is **npm scope control confirmation and package-name transition planning**.
